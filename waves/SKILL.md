...
Model choice comes from `~/.agents/routing.json` (written by the
`setup-lee-engineering` skill): scouts on `scout`, coding workers on
...
- `references/advanced-patterns.md` for row fan-out, tournaments, parallel writes.
...
The full packet contract (objective, non-goals, slice ownership, permitted
paths, acceptance rules, chosen role, and the required return shape) lives in
`references/handoff-format.md` § "Worker prompt packet". End every worker
prompt with the copy-paste ending for its worker type from the same file.

## Advanced patterns

Row-shaped CSV fan-out, generate-and-filter tournaments, parallel writes and
worktrees, native verification surfaces, and escalating beyond one interactive
